  ...
  return (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)("style", {
    dangerouslySetInnerHTML: {
      __html: `
		...
		#${id} .${_utils_data__WEBPACK_IMPORTED_MODULE_1__.prefix} .plyr,
		#${id} .${_utils_data__WEBPACK_IMPORTED_MODULE_1__.prefix} .videoWrapper {
			border-radius: ${radius};
		}
		#${id} .${_utils_data__WEBPACK_IMPORTED_MODULE_1__.prefix} .plyr__video-wrapper {
...
const getExtension = fileName => fileName ? fileName.substring(fileName.lastIndexOf('.') + 1) : '';
const isYoutube = url => {
  if (!url) return null;
...
const RenderVideo = ({
  ...
  (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
    ...
    let observer = null;
    const init = () => {
      player = new Plyr(node, (0,_utils_config__WEBPACK_IMPORTED_MODULE_5__.plyrConfig)(attributes));
      player.on('ready', () => {
        if (muted && autoplay) {
          player.play();
        }
  ...
  const crossOriginProps = captions?.length ? {
  ...
  }), captions?.map((cap, index) => cap.src ? (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)("track", {
    key: index,
    kind: "captions",
    src: cap.src,
    label: cap.label,
